...
class NewsDB implements INewsDB {
    ...
    // Закрытое свойство для хранения экземпляра SQLite3
    private $_db;
    ...
    public function saveNews($title, $category, $description, $source) {
        // 7. Создайте переменную $dt и присвойте ей значение текущих даты и времени в формате timestamp
        $dt = time();

        // 8. Сформируйте строку запроса на добавление новой записи в таблицу msgs, используя переменную $dt и переданные параметры
        ...
        if (!$catRow) {
            $categoryId = 0; // или можно бросить исключение
        } else {
        ...
        $stmt->bindValue(5, $dt, SQLITE3_INTEGER); // используем $dt

        // 9. Выполните запрос и возвратите нужное значение
        $result = $stmt->execute();
        return $result !== false;
    }
